...
void DrawPixel(unsigned long x, unsigned long y);//画点
...
//计算点的坐标
static void CalcCoord(struct ExprNode *Hor_Exp,//横坐标语法树的根节点
	struct ExprNode * Ver_Exp,//纵坐标语法树的根节点
	double &Hor_x,//点横坐标值
	double &Ver_y)//点纵坐标值
{
	double HorCord, VerCord, Hor_tmp;
	HorCord = GetExprValue(Hor_Exp);
	VerCord = GetExprValue(Ver_Exp);//计算原始坐标
	HorCord *= Scale_x;
	VerCord *= Scale_y;
	...
	struct ExprNode *VerPtr)//纵坐标表达式语法树的根节点
{
	...
	{
		CalcCoord(HorPtr, VerPtr, x, y);//实际坐标
		DrawPixel((unsigned long)x, (unsigned long)y);//画点
	}
}
...
void DelExprTree(struct ExprNode *root)
{
	...
	case POWER:
		...
		break;
	case FUNC:
		DelExprTree(root->Content.CaseFunc.Child);//一元
		break;
	default:
		break;
	}
